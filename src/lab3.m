...
disp('ikin:')
target = ikin(10,10,10)
disp('done')

joint1 = [convertToEnc(target(1))];
joint2 = [convertToEnc(target(2))];
joint3 = [convertToEnc(target(3))];
...
try
    ...
    for k = 1:1
        ...
        packet(1) = joint1(k); %Writes setpoint to joint 1
...
l6 = sqrt(x^2 + y^2);
theta(1) = atan2(y, x);
theta(3) =pi - acos((l2^2+l3^2-l4^2)/(2*l2*l3)); 

theta(3) = -abs(theta(3)) + pi/2;

theta(2) = atan2(l5, l6) + asin(l3*sin(pi-theta(3))/l4);
